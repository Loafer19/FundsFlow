...
const positiveTags = computed(() => {
    const tagMap = props.transactions
        .filter((t) => t.amount > 0)
        .reduce((acc, t) => {
            t.tags.forEach((tag) => {
    ...
    const tagMap = props.transactions
        ...
        .reduce((acc, t) => {
            t.tags.forEach((tag) => {
                ...
                acc[tag.id].amount += Math.abs(t.amount)
                acc[tag.id].count += 1
            })
            return acc
        }, {})
...
const generateRandomColor = () => {
    const lightness = Math.random() * (80 - 65) + 65 // 65-80%
    const chroma = Math.random() * (0.2 - 0.14) + 0.14 // 0.14-0.2
...
const positiveChartData = computed(() => ({
    labels: positiveTags.value.map((tag) => tag.emoji + ' ' + tag.title),
    datasets: [
        {
            data: positiveTags.value.map((tag) => tag.amount),
            backgroundColor: positiveTags.value.map(() => generateRandomColor()),
        },
    ],
}))

const negativeChartData = computed(() => ({
    labels: negativeTags.value.map((tag) => tag.emoji + ' ' + tag.title),
    datasets: [
        {
            data: negativeTags.value.map((tag) => tag.amount),
            backgroundColor: negativeTags.value.map(() => generateRandomColor()),
        },
    ],
}))
...
const positiveChartOptions = {
    ...
    plugins: {
        ...
        tooltip: {
            callbacks: {
                label: (context) => {
                    const tag = positiveTags.value[context.dataIndex]
                    return tag.amount + ' - ' + tag.count + ' transactions'
                },
            },
        },
    },
...
const negativeChartOptions = {
    ...
    plugins: {
        ...
        tooltip: {
            callbacks: {
                label: (context) => {
                    const tag = negativeTags.value[context.dataIndex]
                    return tag.amount + ' - ' + tag.count + ' transactions'
                },
            },
        },
